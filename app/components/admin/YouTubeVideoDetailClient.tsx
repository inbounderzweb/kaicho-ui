...
import {
  useYouTubeVideoDetail,
  useCreateYouTubeVideo,
  useUpdateYouTubeVideo,
  useSetYouTubeVideoStatus,
  useDeleteYouTubeVideo,
} from "@/lib/hooks/admin/useYouTubeVideos";
import {
  youtubeVideoFormSchema,
  type YouTubeVideoFormValues,
} from "@/lib/validation/youtubeVideo.schema";
import { parseYouTubeUrl } from "@/lib/utils/youtubeUrl";
  ...
  if (!parsed) {
    return (
      <p className="text-xs text-black/45 dark:text-white/45">
        Enter a valid YouTube video or short link to preview it.
      </p>
    );
  }
  return (
    <div className="space-y-3">
      <p className="text-sm font-semibold">
        ...
        <span className="font-mono text-black/60 dark:text-white/60">{parsed.videoId}</span>
      </p>
      {/* Live preview via YouTube's /embed/ endpoint — browser-only, no server call. */}
      <YouTubeEmbed key={parsed.videoId} videoId={parsed.videoId} />
      <a
...
export default function YouTubeVideoDetailClient({ id }: { id?: string }) {
  const isEdit = Boolean(id);
  const router = useRouter();

  const { data, isLoading, isError, error, refetch } = useYouTubeVideoDetail(id ?? null);
  const createMutation = useCreateYouTubeVideo();
  const updateMutation = useUpdateYouTubeVideo(id ?? "");
  const statusMutation = useSetYouTubeVideoStatus(id ?? "");
  ...
  useEffect(() => {
    ...
    reset({
      ...
      status: video.status === "ARCHIVED" ? "INACTIVE" : video.status,
    });
  }, [video, reset]);
  ...
  const onSubmit = (values: YouTubeVideoFormValues) => {
    const parsed = youtubeVideoFormSchema.parse(values);
    const payload = { url: parsed.url, displayOrder: parsed.displayOrder, status: parsed.status };
    if (isEdit) {
      ...
      createMutation.mutate(payload, {
        onSuccess: (res) => router.push(`/admin/youtube-videos/${res.video.id}`),
      });
    }
  };
  ...
  const runArchive = () => {
    ...
    deleteMutation.mutate(id as string, {
      onSuccess: () => router.push("/admin/youtube-videos"),
      onError: (err) =>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2 text-sm">
        ...
        <span className="font-semibold">{isEdit ? video?.videoId ?? "Edit" : "Add YouTube Video"}</span>
        {video && <StatusBadge status={STATUS_LABEL[video.status]} />}
      </div>
      ...
      {isArchived && (
        <div className={`${CARD} sm:flex sm:items-center sm:justify-between sm:space-y-0`}>
          <p className="text-sm text-black/60 dark:text-white/60">
            This video is archived and hidden from the active list.
          </p>
          <button
            type="button"
